...
function Info1() {
  ...
  const { cateClo, setCateClo, clearCateClo, setIsDetailInfo } =
    useGlobalContext()
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    const { type, brand, material, size, color, gender, productId } = cateClo
    try {
      let res = await axios({
        method: "PUT",
        url: `https://tlcngroup2be.herokuapp.com/seller/product/categoryclothes/${productId}`,
        data: { type, brand, material, size, color, gender },
